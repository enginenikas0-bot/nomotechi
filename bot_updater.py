...
try:
    ...
    if api_key:
        ...
        HAS_AI = True
        print("✅ Gemini AI Connected! (Government Mode)")
    else:
...
def scrape_full_text(url):
    try:
        ...
        if response.status_code == 200:
            ...
            for script in soup(["script", "style"]): script.extract() # Καθαρισμός
            paragraphs = soup.find_all('p')
            full_text = " ".join([p.get_text() for p in paragraphs])
            full_text = re.sub(r'\s+', ' ', full_text).strip()
            if len(full_text) < 200: return "" 
            return full_text[:8500] # Αυξημένο όριο για βαθιά ανάλυση
    except: return ""
    return ""
...
def guess_category_classic(text):
    ...
    if any(w in text for w in ['φεκ', 'εγκυκλιος', 'υπουργικη αποφαση', 'νομος υπ αριθμ']):
...
# --- 4. AI ANALYST (GOVERNMENT GRADE PROMPT) ---
def analyze_article_with_ai(title, full_text):
    if not HAS_AI: return None, None
    
    # ΤΟ ΑΠΟΛΥΤΟ PROMPT
    prompt = f"""
    You are the Chief Information Officer for a Greek Government Portal.
    Analyze the following article deeply.
    
    TITLE: {title}
    TEXT SAMPLE: {full_text[:8000]}
    
    --- PART 1: AUDIENCE ANALYSIS & TAGGING ---
    Determine the primary audience using these STRICT Logic Gates:
    
    1. ENGINEERS_GATE: 
       - Does it involve Construction, Infrastructure, Energy, Zoning (Poleodomia), Ktimatologio, or Technical Studies?
       - IF YES -> Add tag "ENGINEERS".
       
    2. REAL_ESTATE_GATE:
       - Does it involve Property Tax (ENFIA), Rents, Buying/Selling, Golden Visa, or Airbnb rules?
       - IF YES -> Add tag "REAL_ESTATE".
       
    3. LEGAL_GATE (The Strict Filter):
       - Does it involve Court Rulings (StE/Areopagos), Criminal/Civil Law, Lawyer Regulations (DSA), or Justice Reform?
       - CRITICAL: If the topic is CONSTRUCTION or ZONING laws, DO NOT USE "LEGAL". Leave it for Engineers. 
       - ONLY use "LEGAL" if it is purely a matter of Justice/Lawyers.
       - IF Pure Legal -> Add tag "LEGAL".

    4. LEGISLATION_GATE:
       - Is this an Official Text? (FEK, Circular/Egkyklios, Ministerial Decision/YA, Draft Law)?
       - IF YES -> Add tag "LEGISLATION".
       
    5. SOS_GATE:
       - Are there hard deadlines, fines, or expirations? -> Add tag "SOS".

    --- PART 2: EXECUTIVE SUMMARY (GREEK) ---
    - Write a high-level summary in Greek using Bullet Points (•).
    - EXTRACT: Exact dates, Monetary amounts (€), Law Numbers, and Protocol Numbers.
    - Style: Formal, Concise, Authoritative.
    
    --- OUTPUT FORMAT ---
    Strictly follow this format with the ||| separator:
    TAG1, TAG2, TAG3 ||| • Bullet 1... • Bullet 2...
    """
    try:
...
# --- 5. MAIN LOOP (SMART UPDATER) ---
def run():
    print(f"🤖 [NomoTechi Gov-AI v7] Initiating...")
    ...
    for source_name, url in RSS_FEEDS.items():
        try:
            ...
            for entry in feed.entries[:3]: # Top 3 items for speed/quality balance
                print(f"   🔎 Analyst: {entry.title[:30]}...")
                ...
                if entry.link in link_map:
                    # SMART UPDATE (Fix Categories & Summary)
                    row_num = link_map[entry.link]
                    print(f"      ♻️ Re-indexing Row {row_num}: {tags}")
    ...
    print(f"🏁 System Update Complete. Added: {new_items}, Refined: {updated_items}")
